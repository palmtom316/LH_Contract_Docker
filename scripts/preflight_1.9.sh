...
TARGET_REVISION="20260725_remove_bank_receipts"
PREVIOUS_TARGET_REVISION="20260723_expand_system_config"
EARLIER_TARGET_REVISION="20260723_zero_hour_finance_fields"
LEGACY_TARGET_REVISION="20260722_durable_import_jobs"
...
if [ "${MODE}" = before ]; then
    if [ "${revision}" != "${BASE_REVISION}" ] && [ "${revision}" != "${CURRENT_18_REVISION}" ] && [ "${revision}" != "${LEGACY_TARGET_REVISION}" ] && [ "${revision}" != "${EARLIER_TARGET_REVISION}" ] && [ "${revision}" != "${PREVIOUS_TARGET_REVISION}" ] && [ "${revision}" != "${TARGET_REVISION}" ]; then
        echo "Preflight failed: expected ${BASE_REVISION}, ${CURRENT_18_REVISION}, ${LEGACY_TARGET_REVISION}, ${EARLIER_TARGET_REVISION}, ${PREVIOUS_TARGET_REVISION}, or ${TARGET_REVISION}, got ${revision:-missing}" >&2
        exit 1
    fi
else
...
if [ "${MODE}" = after ]; then
    ...
    for table in bank_receipt_batches bank_receipt_items bank_receipt_allocations bank_receipt_match_candidates; do
        exists="$(query "SELECT 1 FROM information_schema.tables WHERE table_name='${table}' LIMIT 1;")"
        [ -z "${exists}" ] || { echo "Preflight failed: bank receipt table still present: ${table}" >&2; exit 1; }
    done
fi
