...
import { getAvailableStock, getMyWarehouseScopes, listLocations, listMaterials, listProjects } from '@/api/warehouse'
...
export function useWarehouseForm() {
  ...
  const selectedWarehouse = computed(() => warehouses.value.find(item => item.id === form.value.warehouse_id))

  async function loadLookups() {
    warehouses.value = await getMyWarehouseScopes()
    projects.value = await listProjects()
    const defaultWarehouse = warehouses.value.find(item => item.is_default) || warehouses.value[0]
    if (defaultWarehouse) {
      form.value.warehouse_id = defaultWarehouse.warehouse_id || defaultWarehouse.id
  ...
  async function refreshAvailable() {
    ...
    const res = await getAvailableStock({ warehouse_id, location_id, project_id, material_id })
    available.value = res.quantity
  }

  watch(() => form.value.warehouse_id, (id) => refreshLocations(id, 'location_id'))
  watch(() => [form.value.warehouse_id, form.value.location_id, form.value.project_id, form.value.material_id], refreshAvailable)
