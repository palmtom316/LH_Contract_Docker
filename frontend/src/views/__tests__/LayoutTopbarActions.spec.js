...
describe('Layout topbar icon actions', () => {
  ...
  it('wires the shared contract query assistant trigger into the desktop shell', () => {
    ...
    expect(topbarActionsSource).toContain('open-contract-query')
    expect(topbarActionsSource).toContain('class="contract-query-trigger"')
    expect(topbarActionsSource).toContain('isContractQueryOpen')
    expect(topbarActionsSource).toContain('showContractQuery')
    expect(layoutSource).toContain('@open-contract-query="openContractQuery"')
    expect(layoutSource).toContain(':is-contract-query-open="uiStore.contractQueryOpen"')
    expect(layoutSource).toContain(':show-contract-query="userStore.canViewUpstreamContracts"')
    ...
    expect(topbarActionsSource).toContain('contract-query-trigger__icon')
    expect(topbarActionsSource).toContain('合同查询</span>')
    expect(topbarActionsSource).toContain('Ctrl + K')
  })
})
