  ...
  <div class="business-dashboard" v-loading="loading">
    ...
    <el-row :gutter="20" class="summary-row">
      ...
      <el-col :xs="24" :sm="12" :md="6">
        <el-card shadow="hover" class="stat-card-modern" style="background: linear-gradient(135deg, #1890FF 0%, #36CFC9 100%);">
          <div class="card-icon-bg"><el-icon><Document /></el-icon></div>
          <div class="card-inner">
            ...
            <div class="card-modern-content">
              ...
              <div class="amount-sub">¥ {{ formatWan(annualUpstreamAmount) }} 万</div>
      ...
      <el-col :xs="24" :sm="12" :md="6">
        <el-card shadow="hover" class="stat-card-modern" style="background: linear-gradient(135deg, #52C41A 0%, #95D475 100%);">
          <div class="card-icon-bg"><el-icon><Money /></el-icon></div>
          <div class="card-inner">
            <div class="card-modern-header">
      ...
      <el-col :xs="24" :sm="12" :md="6">
        <el-card shadow="hover" class="stat-card-modern" style="background: linear-gradient(135deg, #FF4D4F 0%, #F56C6C 100%);">
          <div class="card-icon-bg"><el-icon><Coin /></el-icon></div>
          <div class="card-inner">
            <div class="card-modern-header">
              <span class="title">年度付款总额</span>
              <div class="icon-wrapper"><el-icon><Coin /></el-icon></div>
            </div>
            <div class="card-modern-content">
               <h2 class="amount">
      ...
      <el-col :xs="24" :sm="12" :md="6">
        <el-card shadow="hover" class="stat-card-modern" style="background: linear-gradient(135deg, #FAAD14 0%, #FADB14 100%);">
          <div class="card-icon-bg"><el-icon><Wallet /></el-icon></div>
          <div class="card-inner">
            <div class="card-modern-header">
              <span class="title">年度下游及管理签约</span>
              <div class="icon-wrapper"><el-icon><Wallet /></el-icon></div>
            </div>
            <div class="card-modern-content">
              <h2 class="amount">
                {{ annualDownMgmtCount }} <small>单</small>
              </h2>
               <div class="amount-sub">¥ {{ formatWan(annualDownMgmtAmount) }} 万</div>
    ...
    <el-row :gutter="20" style="margin-top: 20px;" class="summary-row">
      ...
      <el-col :xs="24" :sm="24" :md="12">
        <el-card shadow="hover" class="chart-card" style="height: 228px; display: flex; flex-direction: column;" :body-style="{ flex: '1', overflow: 'hidden', padding: '10px', display: 'flex', flexDirection: 'column' }">
          <div class="card-content" style="flex: 1; display: flex; flex-direction: column; overflow: hidden;">
            <div class="card-header-simple" style="margin-bottom: 5px; flex-shrink: 0;">
        ...
        <el-card shadow="hover" class="chart-card" style="height: 228px;">
          <template #header>
             <div class="card-header-simple">
                 <span>{{ currentMonth ? `${currentMonth}月 ` : '' }}应收账款 (AR)</span>
          ...
          <div class="arap-content">
            <div class="arap-main-value text-primary">¥ {{ formatWan(arStats.outstanding) }} <small>万</small></div>
            <div class="arap-sub">
               <div class="sub-row"><span class="label">应收款</span><span class="val">¥ {{ formatWan(arStats.total_receivable) }} 万</span></div>
               <div class="sub-row"><span class="label">已收款</span><span class="val text-success">¥ {{ formatWan(arStats.total_received) }} 万</span></div>
            </div>
            <el-progress 
      ...
      <el-col :xs="24" :sm="12" :md="6">
        <el-card shadow="hover" class="chart-card" style="height: 228px;">
          <template #header>
             <div class="card-header-simple">
                 <span>{{ currentMonth ? `${currentMonth}月 ` : '' }}应付账款 (AP)</span>
          ...
          <div class="arap-content">
             <div class="arap-main-value text-warning">¥ {{ formatWan(apStats.outstanding) }} <small>万</small></div>
             <div class="arap-sub">
               <div class="sub-row"><span class="label">应付款</span><span class="val">¥ {{ formatWan(apStats.total_payable) }} 万</span></div>
               <div class="sub-row"><span class="label">已付款</span><span class="val text-success">¥ {{ formatWan(apStats.total_paid) }} 万</span></div>
            </div>
            <el-progress 
    ...
    <el-row :gutter="20" style="margin-top: 20px;" class="summary-card-row">
      ...
      <el-col :xs="24" :sm="12" class="stat-col-5">
        <el-card shadow="hover" class="chart-card">
          <template #header>
            <div class="card-header-simple">
              <span>{{ currentMonth ? `${currentMonth}月 ` : '' }}上游概况(类别)</span>
              <el-tag effect="plain" type="primary">
                 {{ upstreamSummary.reduce((acc, cur) => acc + cur.count, 0) }} 单 / ¥{{ formatWan(upstreamSummary.reduce((acc, cur) => acc + cur.amount, 0)) }} 万
              </el-tag>
            </div>
          </template>
          <div class="card-list-modern">
             <div v-for="item in upstreamSummary.slice(0, 20)" :key="item.name" class="list-item">
               ...
               <span class="item-value">{{ item.count }}单 / ¥{{ formatWan(item.amount) }}万</span>
             </div>
          </div>
        </el-card>
      </el-col>
      ...
      <el-col :xs="24" :sm="12" class="stat-col-5">
        <el-card shadow="hover" class="chart-card">
          <template #header>
            <div class="card-header-simple">
              <span>{{ currentMonth ? `${currentMonth}月 ` : '' }}上游概况(公司)</span>
              <el-tag effect="plain" type="primary" color="#f3e5f5" style="color: #8e44ad; border-color: #d7bde2;">
                 {{ upstreamCompanySummary.reduce((acc, cur) => acc + cur.count, 0) }} 单 / ¥{{ formatWan(upstreamCompanySummary.reduce((acc, cur) => acc + cur.amount, 0)) }} 万
              </el-tag>
            </div>
          </template>
          <div class="card-list-modern">
      ...
      <el-col :xs="24" :sm="12" class="stat-col-5">
        <el-card shadow="hover" class="chart-card">
          <template #header>
            <div class="card-header-simple">
              <span>{{ currentMonth ? `${currentMonth}月 ` : '' }}下游合同概况</span>
              <el-tag effect="plain" type="danger">
                 {{ downstreamSummary.reduce((acc, cur) => acc + cur.count, 0) }} 单 / ¥{{ formatWan(downstreamSummary.reduce((acc, cur) => acc + cur.amount, 0)) }} 万
              </el-tag>
            </div>
          </template>
          <div class="card-list-modern">
      ...
      <el-col :xs="24" :sm="12" class="stat-col-5">
        <el-card shadow="hover" class="chart-card">
          <template #header>
            <div class="card-header-simple">
              <span>{{ currentMonth ? `${currentMonth}月 ` : '' }}管理合同概况</span>
              <el-tag effect="plain" type="warning">
                 {{ managementSummary.reduce((acc, cur) => acc + cur.count, 0) }} 单 / ¥{{ formatWan(managementSummary.reduce((acc, cur) => acc + cur.amount, 0)) }} 万
              </el-tag>
            </div>
          </template>
          <div class="card-list-modern">
      ...
      <el-col :xs="24" :sm="12" class="stat-col-5">
        <el-card shadow="hover" class="chart-card">
          <template #header>
            <div class="card-header-simple">
              <span>{{ currentMonth ? `${currentMonth}月 ` : '' }}无合同费用</span>
              <el-tag effect="plain" type="info">
                 {{ expenseSummary.reduce((acc, cur) => acc + cur.count, 0) }} 笔 / ¥{{ formatWan(expenseSummary.reduce((acc, cur) => acc + cur.value, 0)) }} 万
              </el-tag>
            </div>
          </template>
          <div class="card-list-modern">
             <div v-for="item in expenseSummary.slice(0, 20)" :key="item.name" class="list-item">
               <span class="item-name">{{ item.name }}</span>
               <span class="item-value">{{ item.count }}笔 / ¥{{ formatWan(item.value) }}万</span>
             </div>
          </div>
        </el-card>
      </el-col>
...
import { Document, Money, Wallet, Coin, TrendCharts, DataAnalysis, InfoFilled, Refresh } from '@element-plus/icons-vue'
