...
describe('Business chart composition', () => {
  ...
  it('keeps the year/month filter controls and refresh action aligned on one shared control axis', () => {
    ...
    expect(businessSource).toContain('class="dashboard-filter-bar__controls"')
    expect(businessSource).toContain('.dashboard-filter-bar__controls {')
    expect(businessSource).toContain('display: flex;')
    expect(businessSource).toContain('flex-wrap: wrap;')
    expect(businessSource).toContain('align-items: center;')
    expect(businessSource).toContain('.dashboard-filter-bar__controls > * {')
    expect(businessSource).toContain('flex: 0 0 184px;')
    expect(businessSource).toContain('.dashboard-filter-bar__controls :deep(.el-button) {')
    expect(businessSource).toContain('flex: 0 0 auto;')
    expect(businessSource).not.toContain('app-filter-bar__actions--inline')
  })
})
