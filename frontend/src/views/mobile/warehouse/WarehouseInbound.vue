  ...
  <div class="wh-form">
    <van-form @submit="submit">
      <van-cell-group inset>
        ...
        <van-cell v-for="item in materials" :key="item.id" :title="`${item.code} ${item.name}`" :label="item.specification" clickable @click="selectMaterial(item)" />
        <van-field v-model="form.quantity" type="number" label="数量" placeholder="大于 0" />
...
const businessOptions = [
  { name: '采购入库', value: 'PURCHASE' },
  { name: '领用退回', value: 'RETURN' },
  { name: '拆除回收', value: 'DEMOLITION' },
  { name: '期初', value: 'OPENING' }
]
