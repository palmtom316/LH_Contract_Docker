          ...
          >
            <header class="invoice-card-header">
              ...
              <div class="invoice-card-statuses">
                ...
                <el-tag
                  size="small"
                  :type="matchTagType(item.match_status)"
                  effect="light"
                >
            ...
            <section class="invoice-project-band">
              <div>
                <span>建筑项目名称</span>
                <strong>{{
                  item.construction_project_name || "未识别"
                }}</strong>
              </div>
              <div>
                <span>项目名称</span>
            ...
            <dl class="invoice-detail-grid">
              ...
              <div>
                <dt>开票日期</dt>
                <dd>{{ item.invoice_date || "-" }}</dd>
              </div>
              <div>
                <dt>发票类型</dt>
                <dd>{{ item.invoice_type || "-" }}</dd>
              </div>
              <div>
                <dt>税率</dt>
                <dd>{{ formatTaxRate(item.tax_rate) }}</dd>
              </div>
              <div>
                <dt>税额</dt>
                <dd>{{ formatOptionalAmount(item.tax_amount) }}</dd>
              </div>
            </dl>

            <section
              class="invoice-match-panel"
              :class="{ 'is-empty': !item.candidates?.length }"
            >
              ...
              <div v-if="item.candidates?.length" class="matched-contract-list">
                <div
                  ...
                  :key="candidate.id"
                  class="matched-contract"
                >
                  <strong
                    >[{{ candidate.contract_serial_number || "-" }}]
                    {{ candidate.contract_name || "合同信息缺失" }}</strong
                  >
                  <span v-if="candidate.contract_code">{{
                    candidate.contract_code
                  }}</span>
                </div>
              </div>
              <span v-else class="invoice-match-empty"
                >未按建筑项目名称找到合同，可手动搜索选择</span
              >
            </section>
      ...
      >
        <el-form label-width="100px">
          <el-form-item label="方向">
            <el-radio-group
              v-model="allocationForm.direction"
          ...
          <el-form-item
            v-if="allocationForm.direction === 'downstream'"
            label="合同类型"
          >
            <el-radio-group
              v-model="allocationForm.contract_type"
              @change="handleDirectionChange"
            >
              <el-radio-button value="downstream">下游合同</el-radio-button>
              <el-radio-button value="management">管理合同</el-radio-button>
              <el-radio-button value="zero_hour_labor"
                >零星用工</el-radio-button
              >
            </el-radio-group>
          </el-form-item>
...
import { getZeroHourLaborList } from "@/api/zeroHourLabor";
...
async function openAllocation(row) {
  ...
  allocationForm.direction =
    row.direction === "downstream" ? "downstream" : "upstream";
  ...
  allocationForm.description =
    row.construction_project_name || row.project_name || row.remarks || "";
...
function contractOptionLabel(contract) {
  return contract.contract_name
    ? `[${contract.serial_number || "-"}] ${contract.contract_name} (${contract.contract_code || "-"})`
    : `[零星用工 #${contract.id}] ${contract.dispatch_unit || "未填写派工单位"} (${contract.labor_date || "-"})`;
}
...
function mergeContractOptions(...groups) {
  ...
  groups
    .flat()
    .filter(Boolean)
    .forEach((contract) => merged.set(contract.id, contract));
...
function handleDirectionChange() {
  ...
  if (allocationForm.direction === "upstream")
    allocationForm.contract_type = "upstream";
  contractOptions.value = recommendedContracts();
}
...
function searchContracts(query) {
  ...
  if (!query) {
    const selected = contractOptions.value.find(
      (contract) => contract.id === allocationForm.contract_id,
    );
    contractOptions.value = mergeContractOptions(
  ...
  contractSearchTimer = window.setTimeout(async () => {
    ...
    try {
      const loader =
        allocationForm.direction === "upstream"
          ? getUpstreamContracts
          : allocationForm.contract_type === "management"
            ? getManagementContracts
            : allocationForm.contract_type === "zero_hour_labor"
              ? getZeroHourLaborList
...
async function saveAllocation() {
  ...
  if (allocationForm.direction === "upstream") {
    ...
    data[targetField] = allocationForm.contract_id;
  }
...
async function ignore(row) {
  try {
    const result = await ElMessageBox.prompt("请输入忽略原因", "忽略发票", {
      inputValidator: (value) => value?.length > 1,
    });
    await ignoreInvoiceItem(row.id, result.value);
    ElMessage.success("发票已忽略");
    await refreshItems();
  } catch {}
}
...
async function deleteFailed(row) {
  try {
    ...
    await deleteFailedInvoiceItem(row.id);
