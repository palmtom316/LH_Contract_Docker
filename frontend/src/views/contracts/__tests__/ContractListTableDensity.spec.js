...
describe('contract desktop tables', () => {
  ...
  it('uses shared wrapping hooks instead of inline white-space style objects', () => {
    ...
    expect(upstreamSource).not.toContain('max-height: 4.5em')
    expect(downstreamSource).not.toContain('max-height: 4.5em')
    expect(managementSource).not.toContain('max-height: 4.5em')
    expect(upstreamSource).not.toContain('-webkit-line-clamp: 3')
  })
})
