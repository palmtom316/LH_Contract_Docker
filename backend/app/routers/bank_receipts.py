...
from app.schemas.bank_receipt import (
    BankReceiptBatchResponse,
    BankReceiptItemResponse,
    ReceiptAllocationCreate,
    ReceiptAllocationResponse,
    ClearReceiptRequest,
    BankReceiptReviewUpdate,
)
...
def service(db: AsyncSession = Depends(get_db)):
    return BankReceiptService(db)


@router.post(
    "/batches",
    response_model=BankReceiptBatchResponse,
...
async def upload(
    ...
    return await svc.upload(file, user)


@router.get("/batches", response_model=list[BankReceiptBatchResponse])
async def batches(
    user: User = Depends(require_permission(Permission.VIEW_PAYMENTS)),
    svc: BankReceiptService = Depends(service),
):
...
async def items(
    ...
    return await svc.items(batch_id)
...
async def contracts(
    ...
    return await svc.search(q, direction)


@router.post("/items/{item_id}/allocations", response_model=ReceiptAllocationResponse)
async def allocation(
    item_id: int,
    data: ReceiptAllocationCreate,
    user: User = Depends(require_permission(Permission.CREATE_PAYMENTS)),
    svc: BankReceiptService = Depends(service),
):
    return await svc.allocate(item_id, data)


@router.put(
    "/items/{item_id}/allocations/{allocation_id}",
    response_model=ReceiptAllocationResponse,
)
async def update_allocation(
    ...
    return await svc.update_allocation(item_id, allocation_id, data)
...
async def delete_allocation(
    ...
    await svc.delete_allocation(item_id, allocation_id)
    return Response(status_code=204)
...
async def review(
    item_id: int,
    data: BankReceiptReviewUpdate,
    user: User = Depends(require_permission(Permission.EDIT_PAYMENTS)),
    svc: BankReceiptService = Depends(service),
):
    return await svc.review(item_id, data, user)
...
async def confirm(
    ...
    return await svc.confirm(item_id, user)
...
async def receipt_file(
    ...
    response = get_minio_client().get_object(
        settings.MINIO_BUCKET_CONTRACTS, item.file_key
    )
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f'inline; filename="receipt-{item.id}.pdf"'},
    )
