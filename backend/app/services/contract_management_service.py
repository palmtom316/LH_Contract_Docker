...
class ContractManagementService(BaseContractService[ContractManagement]):
    ...
    async def list_contracts(
        ...
        if upstream_contract_id:
            query = query.where(
                ContractManagement.upstream_contract_id == upstream_contract_id
            )
        ...
        # Pagination
        query = (
            query.order_by(desc(ContractManagement.created_at))
    ...
    async def create_contract(
        self, contract_in: ContractManagementCreate, user: User
    ) -> ContractManagement:
        ...
        if not data.get("contract_code") or data["contract_code"].strip() == "":
            ...
            data["contract_code"] = await code_generator.generate_management_code(
                sign_date
            )
    ...
    async def update_contract(
        self, contract_id: int, contract_in: ContractManagementUpdate, user: User
    ) -> ContractManagement:
