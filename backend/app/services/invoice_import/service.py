...
from app.services.invoice_import.archive import (
    ExtractedInvoicePackage,
    UnsafeArchiveError,
    extract_invoice_archives,
)
from app.services.invoice_import.matching import (
    InvoiceMatchService,
    build_dedupe_key,
    detect_direction,
)
...
class InvoiceImportService:
    ...
    async def claim_next_batch(self) -> tuple[int, str] | None:
        ...
        batch = await self.db.scalar(
            ...
            .where(
                or_(
                    InvoiceImportBatch.status == "uploaded",
                    (InvoiceImportBatch.status == "processing")
                    & (InvoiceImportBatch.job_lease_until < now),
                ),
                or_(
                    InvoiceImportBatch.job_next_attempt_at.is_(None),
                    InvoiceImportBatch.job_next_attempt_at <= now,
                ),
            )
            .order_by(InvoiceImportBatch.created_at)
        ...
        batch.job_lease_until = now + timedelta(seconds=_JOB_LEASE_SECONDS)
        batch.job_next_attempt_at = None
        batch.job_attempts = (batch.job_attempts or 0) + 1
        await self.db.commit()
        return batch.id, token

    async def mark_job_failure(self, batch_id: int, token: str, exc: Exception) -> None:
        batch = await self.db.scalar(
            select(InvoiceImportBatch)
            .where(InvoiceImportBatch.id == batch_id)
            .with_for_update()
        )
        if not batch or batch.job_worker_token != token:
            await self.db.rollback()
            return
        ...
        if (batch.job_attempts or 0) < _JOB_MAX_ATTEMPTS:
            ...
            batch.job_next_attempt_at = None
            logger.error(
                "Invoice import batch %s exhausted retries: %s", batch.id, message
            )
        await self.db.commit()

    async def list_batches(
        self, current_user: User | None = None
    ) -> list[InvoiceImportBatch]:
        confirmed_count = (
    ...
    async def delete_batch(self, batch_id: int, current_user: User) -> None:
        ...
        if any(
            ...
            or any(
                allocation.status == "confirmed" or allocation.formal_invoice_id
    ...
    async def create_allocation(
        self, item_id: int, allocation_in: AllocationCreate, user: User
    ) -> InvoiceImportAllocation:
        ...
        allocation = InvoiceImportAllocation(
            item_id=item.id, created_by=user.id, **data
        )
    ...
    async def update_allocation(
        ...
        result = await self.db.execute(
            ...
            .options(
                selectinload(InvoiceImportAllocation.item).selectinload(
                    InvoiceImportItem.batch
                )
            )
            .where(InvoiceImportAllocation.id == allocation_id)
        )
        allocation = result.scalar_one_or_none()
        if not allocation or (
            not _can_access_all_imports(user)
            and allocation.item.batch.uploaded_by != user.id
        ):
            raise ResourceNotFoundError(
                resource_type="发票分摊", resource_id=allocation_id
            )
        if allocation.status != "draft":
            raise ValidationError(
                message="只能修改草稿分摊", field_errors={"status": "只能修改草稿分摊"}
            )

        update_data = allocation_in.model_dump(exclude_unset=True)
        for key, value in update_data.items():
            setattr(allocation, key, value)
        self._validate_allocation_target(
            allocation.direction,
            allocation.upstream_contract_id,
    ...
    async def ignore_item(
        ...
        item = await self._get_item_for_user(item_id, user)
        if item.confirmation_status != "draft" or item.parse_status in {
            "processing",
            "failed",
        }:
            raise ValidationError(message="当前状态不能忽略")
        item.confirmation_status = "ignored"
    ...
    async def clear_posting(
        ...
        if item.confirmation_status == "cleared":
            return item
        if item.confirmation_status != "confirmed":
            raise ValidationError(message="只有已挂账发票可以清除挂账")
        snapshot = []
        for allocation in item.allocations:
            ...
            snapshot.append(
                {
                    ...
                    "formal_invoice_id": allocation.formal_invoice_id,
            ...
            if allocation.formal_invoice_id:
                ...
                if formal:
                    if isinstance(formal, ZeroHourLaborInvoice):
    ...
    async def delete_failed_item(self, item_id: int, user: User) -> None:
        ...
        if (
            ...
            or item.confirmation_status != "draft"
            or any(a.formal_invoice_id for a in item.allocations)
        ):
        ...
        try:
            ...
            await self.db.commit()
            raise ValidationError(message="源文件删除失败，数据库记录已保留，请重试")
        await self.db.delete(item)
        await create_audit_log(self.db, user, "DELETE", "发票失败文件", item_id)
        await self.db.commit()
    ...
    async def create_batch_from_upload(
        ...
        if not filename.lower().endswith(".zip"):
            raise ValidationError(
                message="文件格式错误", field_errors={"file": "只支持 zip 压缩包"}
            )
        ...
        while True:
            ...
            if total_size > settings.INVOICE_IMPORT_MAX_ARCHIVE_SIZE:
                raise ValidationError(
                    message="文件过大",
                    field_errors={"file": "发票批次压缩包超过大小限制"},
                )
            content.write(chunk)
        ...
        object_key = (
            f"invoices/imports/batches/{now.strftime('%Y/%m')}/{uuid.uuid4()}.zip"
        )
        ...
        await self._put_bytes_to_minio(
            batch.archive_file_key, content_bytes, "application/zip"
        )
        try:
            self.db.add(batch)
            await self.db.commit()
    ...
    async def process_uploaded_batch(
        self, batch_id: int, worker_token: str | None = None
    ) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="invoice-import-") as tmp:
            work_dir = Path(tmp)
            try:
                packages = extract_invoice_archives(
                    BytesIO(content), work_dir, batch.original_filename
                )
    ...
    async def _process_package(
        self, batch: InvoiceImportBatch, package: ExtractedInvoicePackage
    ) -> bool:
        ...
        pdf_key = await self._store_package_file(
            batch, package.pdf_path, "application/pdf"
        )
        ofd_key = await self._store_package_file(
            batch, package.ofd_path, "application/octet-stream"
        )
        xml_key = await self._store_package_file(
            batch, package.xml_path, "application/xml"
        )
    ...
    async def _get_item_for_user(self, item_id: int, user: User) -> InvoiceImportItem:
        ...
        item = result.scalar_one_or_none()
        if not item or (
            not _can_access_all_imports(user) and item.batch.uploaded_by != user.id
        ):
            raise ResourceNotFoundError(resource_type="导入发票", resource_id=item_id)
        return item
    ...
    def _validate_allocation_target(
        self,
        direction: str,
        upstream_contract_id: int | None,
        downstream_contract_id: int | None,
        management_contract_id: int | None = None,
    ) -> None:
        if direction == InvoiceDirection.UPSTREAM or direction == "upstream":
            if (
                not upstream_contract_id
                or downstream_contract_id
                or management_contract_id
            ):
                raise ValidationError(
        ...
        if direction == InvoiceDirection.DOWNSTREAM or direction == "downstream":
            if (
                bool(downstream_contract_id) == bool(management_contract_id)
                or upstream_contract_id
            ):
                raise ValidationError(
                    message="进项发票分摊目标无效",
                    field_errors={"downstream_contract_id": "请选择下游或管理合同之一"},
                )
            return
        raise ValidationError(
            message="分摊方向必须为上游或下游",
            field_errors={"direction": "分摊方向无效"},
        )

    async def _store_package_file(
        self, batch: InvoiceImportBatch, path: Path | None, content_type: str
    ) -> str | None:
        if not path:
            return None
        suffix = path.suffix.lower() or ".bin"
        object_key = (
            f"invoices/imports/items/{batch.batch_number}/{uuid.uuid4()}{suffix}"
        )
    ...
    async def _find_duplicate(self, dedupe_key: str) -> InvoiceImportItem | None:
        result = await self.db.execute(
            select(InvoiceImportItem)
            .where(
                InvoiceImportItem.dedupe_key == dedupe_key,
                InvoiceImportItem.duplicate_of_item_id.is_(None),
            )
            .limit(1)
        )
        return result.scalar_one_or_none()

    async def _put_bytes_to_minio(
