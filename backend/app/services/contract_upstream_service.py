...
class ContractUpstreamService(BaseContractService[ContractUpstream]):
            ...
            "zero_hour_labors": [
                {
                    "id": row.id,
                    "labor_date": (
                        row.labor_date.isoformat() if row.labor_date else None
                    ),
    ...
    async def list_contracts(
        ...
        invoices_sub = (
            ...
            .where(
                FinanceUpstreamInvoice.contract_id == ContractUpstream.id,
                FinanceUpstreamInvoice.posting_status == "active",
            )
            .correlate(ContractUpstream)
            .scalar_subquery()
        )
        ...
        receipts_sub = (
            ...
            .where(
                FinanceUpstreamReceipt.contract_id == ContractUpstream.id,
        ...
        # Pagination
        query = (
            query.order_by(desc(ContractUpstream.created_at))
        ...
        rows = (
            result.all()
        )  # returns list of (ContractUpstream, t_rec, t_inv, t_receipt, t_settle)

        # 3. Wrap results
        items = [ContractWrapper(r[0], r[1], r[2], r[3], r[4]) for r in rows]
    ...
    async def list_all_contracts(
        ...
        query = select(
            ...
            receipts_sub.label("total_received"),
            settlements_sub.label("total_settlement"),
        ).options(selectinload(ContractUpstream.settlements))
        ...
        return [ContractWrapper(r[0], r[1], r[2], r[3], r[4]) for r in rows]

    async def create_contract(
        self, contract_in: ContractUpstreamCreate, user: User
    ) -> ContractUpstream:
        ...
        if not data.get("contract_code") or data["contract_code"].strip() == "":
            ...
            data["contract_code"] = await code_generator.generate_upstream_code(
                sign_date
            )
            logger.info(
                f"Auto-generated contract code: {data['contract_code']} (based on sign_date: {sign_date})"
            )
        ...
        try:
            ...
            existing_contract = await self.check_contract_code_exists(
                data["contract_code"]
            )
    ...
    async def update_contract(
        self, contract_id: int, contract_in: ContractUpstreamUpdate, user: User
    ) -> ContractUpstream:
        ...
        old_values = {
            ...
            for k in contract_in.model_dump(exclude_unset=True).keys()
        ...
        if (
            ...
            and update_data["serial_number"] != contract.serial_number
        ):
            if await self.check_serial_number_exists(
                update_data["serial_number"], exclude_id=contract_id
            ):
                raise DuplicateRecordError(
                    resource_type="合同序号",
                    field_name="serial_number",
        ...
        if (
            "contract_code" in update_data
            and update_data["contract_code"] != contract.contract_code
        ):
            existing_contract = await self.check_contract_code_exists(
                update_data["contract_code"], exclude_id=contract_id
            )
            if existing_contract:
                raise AppException(
                    error_code=ErrorCode.CONTRACT_NUMBER_EXISTS,
    ...
    async def refresh_contract_status(self, contract_id: int) -> None:
        ...
        total_received = sum(
            r.amount or 0 for r in contract.receipts if r.posting_status != "cleared"
        )
        total_receivable = sum(r.amount or 0 for r in contract.receivables)

        new_status = calculate_contract_status(
            contract, total_settlement, total_received, total_receivable
        )
    ...
    async def bulk_create_from_import(
        self, contracts_data: List[Dict], user_id: int
    ) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                existing = await self.db.execute(
                    select(ContractUpstream).where(
                        ContractUpstream.contract_code == row["contract_code"]
                    )
                )
                if existing.scalar_one_or_none():
                    errors.append(
                        f"行 {idx+1}: 合同编号 '{row['contract_code']}' 已存在"
                    )
                    continue
                ...
                contract = ContractUpstream(**row, created_by=user_id)
                self.db.add(contract)
                success_count += 1
